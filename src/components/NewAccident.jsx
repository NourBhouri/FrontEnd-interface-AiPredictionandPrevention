import React from "react";
import "./NewAccident.css";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
  faChartLine,
  faUserPlus,
  faExclamationTriangle,
  faSignOutAlt,
  faBell,
} from "@fortawesome/free-solid-svg-icons";
import logo from "../assets/draexlmaier-logo.png";

export default function NewAccident() {
  return (
    <div className="dashboard">
      <div className="sidebar">
        <h2 className="title">Dashboard</h2>
        <div className="menu">
          <button className="menu-btn">
            <FontAwesomeIcon icon={faChartLine} />
            Visualization
          </button>
          <button className="menu-btn ">
            <FontAwesomeIcon icon={faUserPlus} />
            New Employee
          </button>
          <button className="menu-btn">
            <FontAwesomeIcon icon={faExclamationTriangle} />
            Add Accident
          </button>
          <button className="menu-btn">
            <FontAwesomeIcon icon={faSignOutAlt} />
            Log Out
          </button>
        </div>
        <img src={logo} alt="DRÄXLMAIER Logo" className="logo-img" />
      </div>

      <div className="main">
        <div className="top-bar">
          <div className="search-container">
            <FontAwesomeIcon icon="search" className="search-icon" />
            <input className="search" placeholder="Search" />
          </div>
          <FontAwesomeIcon icon={faBell} className="icon" />
          <div className="user-circle"></div>
          <div className="user-info">
            <span>User1</span>
            <small>Admin</small>
          </div>
        </div>

        <h1 className="visual-title">New Accident</h1>

        <div className="form-container">
          <div className="form-grid">
            {[
              "Date",
              "Gender",
              "Process",
              "Location",
              "Segment",
              "Department",
              "Operation Mode",
              "Hazard source",
              "Machine Type",
              "Type of Injury",
              "Injury Body Part",
              "Accident Type",
              "Repeat incident",
              "Risk Factor",
              "Lost Days",
              "Maximum Possible Damage Occured",
              "Recruitment date",
              "SMS",
              "Accident shift",
            ].map((label) => (
              <div key={label} className="dropdown-group">
                <label>{label}</label>
                <select> 
                  {label === "Gender" ? (
                    <>
                      <option>Select Gender</option>
                      <option value="male">Male</option>
                      <option value="female">Female</option>
                    </>
                  ) : label === "Location" ? (
                    <>
                      <option value="">Select Location</option>
                      <option value="Eljem">Eljem</option>
                      <option value="Jemmal">Jemmal</option>
                      <option value="Rosslyn">Rosslyn</option>
                      <option value="Siliana">Siliana</option>
                      <option value="SousseMETS">SousseMETS</option>
                      <option value="SousseUATS">SousseUATS</option>
                    </>
                  ) : label === "Process" ? (
                    <>
                      <option value="">Select Process</option>
                      <option value="I - 4008 - assembly">
                        I - 4008 - assembly
                      </option>
                      <option value="I - 4023 - foldback">
                        I - 4023 - foldback
                      </option>
                      <option value="E - 7015 extracting,_laying-up_and_handle - wire_/_module_/wire_harness">
                        E - 7015 extracting,_laying-up_and_handle -
                        wire_/_module_/wire_harness
                      </option>
                      <option value="E - 8001 commissioning">
                        E - 8001 commissioning
                      </option>
                      <option value="I - 3001 - drilling - vacuum_holes">
                        I - 3001 - drilling - vacuum_holes
                      </option>
                      <option value="I - 3013 - cutting - manual">
                        I - 3013 - cutting - manual
                      </option>
                      <option value="I - 7009 - commissioning">
                        I - 7009 - commissioning
                      </option>
                      <option value="I - 3013 - cutting - manual">
                        I - 3013 - cutting - manual
                      </option>
                      <option value="E - 8001 commissioning">
                        E - 8001 commissioning
                      </option>
                      <option value="I - 3003 - turning">
                        I - 3003 - turning
                      </option>
                      <option value="other">other</option>
                      <option value="E - 7001 walking time_/_assembly line">
                        E - 7001 waI - 4014 - pressing_(joining)-
                        ironing_pressing
                      </option>
                      <option value="I - 4016 - screwing">
                        I - 4016 - screwing
                      </option>
                      <option value="I - 8004 - internal_transport">
                        I - 8004 - internal_transport
                      </option>
                      <option value="I - 5007 - painting - painting - silver_coating_badge">
                        I - 5007 - painting - painting - silver_coating_badge
                      </option>
                      <option value="I - 4007 - gluing - assembly_bonding">
                        I - 4007 - gluing - assembly_bonding
                      </option>
                      <option value="I - 1001 - blow_molding">
                        I - 1001 - blow_molding
                      </option>
                      <option value="E - 2001 welding - ultrasonic - wire-cord">
                        E - 2001 welding - ultrasonic - wire-cord
                      </option>
                      <option value="I - 7003 - measurement">
                        I - 7003 - measurement
                      </option>
                      <option value="I - 7005 - testing">
                        I - 7005 - testing
                      </option>
                      <option value="I - 3022 - cutting - machine">
                        I - 3022 - cutting - machine
                      </option>
                      <option value="E - 7002 testing - function_electrical">
                        E - 7002 testing - function_electrical
                      </option>
                      <option value="I - 3020 - cutting - roll_goods">
                        I - 3020 - cutting - roll_goods
                      </option>
                      <option value="I - 1021 - injection_compression_molding">
                        I - 1021 - injection_compression_molding
                      </option>
                      <option value="I - 8001 - supply_for_transport_to_next_plant">
                        I - 8001 - supply_for_transport_to_next_plant
                      </option>
                      <option value="I - 7011 - cleaning">
                        I - 7011 - cleaning
                      </option>
                      <option value="I - 4015 - pressing_(joining) - roll_pressing">
                        I - 4015 - pressing_(joining) - roll_pressing
                      </option>
                      <option value="E - 4001 crimping - contact_part">
                        E - 4001 crimping - contact_part
                      </option>
                      <option value="I - 8002 - preparation_raw_materials_/_components">
                        I - 8002 - preparation_raw_materials_/_components
                      </option>
                      <option value="E - 4003 assembly - comb_conectro_/fuse_beam">
                        E - 4003 assembly - comb_conectro_/fuse_beam
                      </option>
                      <option value="E - 4015 crimping - doublestrike - wire-contact_part">
                        E - 4015 crimping - doublestrike - wire-contact_part
                      </option>
                      <option value="E - 4016 assembly - corrugated_pipe_/_sleeving">
                        E - 4016 assembly - corrugated_pipe_/_sleeving
                      </option>
                      <option value="I - 7004 - reworking">
                        I - 7004 - reworking
                      </option>
                      <option value="E - 3001 skinning_/strip/_removing - removal - wire_insulation">
                        E - 3001 skinning_/strip/_removing - removal -
                        wire_insulation
                      </option>
                      <option value="E - 3002 cutting - wire">
                        E - 3002 cutting - wire
                      </option>
                      <option value="I - 8001 - supply_for_transport_to_next_plant">
                        I - 8001 - supply_for_transport_to_next_plant
                      </option>
                      <option value="I - 4022 - staple">
                        I - 4022 - staple
                      </option>
                      <option value="I - 1007 - extrusion - back-pressing">
                        I - 1007 - extrusion - back-pressing
                      </option>
                      <option value="I - 3002 - cutting - leather">
                        I - 3002 - cutting - leather
                      </option>
                      <option value="E-Pre Assembly - Detail Planning Group DPG">
                        E-Pre Assembly - Detail Planning Group DPG
                      </option>
                      <option value="E-Assembly - Item number">
                        E-Assembly - Item number
                      </option>
                      <option value="E-order picking - Customer order">
                        E-order picking - Customer order
                      </option>
                      <option value="Logistics">Logistics</option>
                      <option value="Office">Office</option>
                      <option value="I-Production step 3 - lamination and finishing processes - Production cell">
                        I-Production step 3 - lamination and finishing processes
                        - Production cell
                      </option>
                      <option value="E-Cable Cutting - cable or cable-set">
                        E-Cable Cutting - cable or cable-set
                      </option>
                      <option value="I - 8001 - supply_for_transport_to_next_plant">
                        I - 8001 - supply_for_transport_to_next_plant
                      </option>
                      <option value="I-Production step 5 - sequenced assembly - Production island">
                        I-Production step 5 - sequenced assembly - Production
                        island
                      </option>
                      <option value="I-Production step 1 - carrier + pattern Production - technology shop">
                        I-Production step 1 - carrier + pattern Production -
                        technology shop
                      </option>
                      <option value="I-Production step 2 - PrProductionration processes - technologie shop">
                        I-Production step 2 - PrProductionration processes -
                        technologie shop
                      </option>
                      <option value="I-Production step 4 - bulk assembly - Production island">
                        I-Production step 4 - bulk assembly - Production island
                      </option>
                      <option value="E-Staging Assembly - Item number">
                        E-Staging Assembly - Item number
                      </option>
                      <option value="E-Commissioning - Customer order">
                        E-Commissioning - Customer order
                      </option>
                      <option value="E-Staging Assembly - FP-Number">
                        E-Staging Assembly - FP-Number
                      </option>
                      <option value="E-Final Assembly - Customer order">
                        E-Final Assembly - Customer order
                      </option>
                      <option value="I-Production step 1 - carrier + pattern Production - technologie shop">
                        I-Production step 1 - carrier + pattern Production -
                        technologie shop
                      </option>
                      <option value="E-Staging Pre-assembly - Detail Planning Group DPG">
                        E-Staging Pre-assembly - Detail Planning Group DPG
                      </option>
                      <option value="I-Production step 2 - Prefabrication processes - technology shop">
                        I-Production step 2 - Prefabrication processes -
                        technology shop
                      </option>
                      <option value="I-Production step 4 - bulk assembly - production island">
                        I-Production step 4 - bulk assembly - production island
                      </option>
                      <option value="I-Production step 3 - lamination and finishing processes - production cell">
                        I-Production step 3 - lamination and finishing processes
                        - production cell
                      </option>
                      <option value="I-Production step 5 - sequenced assembly - production island">
                        I-Production step 5 - sequenced assembly - production
                        island
                      </option>
                      <option value="C-Back End (assembly)">
                        C-Back End (assembly)
                      </option>
                      <option value="C-Mid End (injection molding / stamping - bending ...)">
                        C-Mid End (injection molding / stamping - bending ...)
                      </option>
                      <option value="I-Production step 4 - bulk assembly - production cell">
                        I-Production step 4 - bulk assembly - production cell
                      </option>
                    </>
                  ) : label === "Segment" ? (
                    <>
                      <option>Select Segment</option>
                      <option value="Electric">Electric</option>
                      <option value="ES">ES</option>
                      <option value="Interior">Interior</option>
                      <option value="IS">IS</option>
                      <option value="SY">SY</option>
                    </>
                  ) : label === "Department" ? (
                    <>
                      <option>Select Department</option>
                      <option value="AB4">AB4</option>
                      <option value="Administration">Administration</option>
                      <option value="Annex KSIBA">Annex KSIBA</option>
                      <option value="assemly">assembly</option>
                      <option value="Bereit">Bereit</option>
                      <option value="Bx8">Bx8</option>
                      <option value="changing room">changing room</option>
                      <option value="FBB">FBB</option>
                      <option value="Foaming area">Foaming area</option>
                      <option value="IE">IE</option>
                      <option value="IR">IR</option>
                      <option value="KS">KS</option>
                      <option value="Logistics">Logistics</option>
                      <option value="Maintenance">Maintenance</option>
                      <option value="Methalle">Methalle</option>
                      <option value="MM">MM</option>
                      <option value="MR">MR</option>
                      <option value="Nacherbeit">Nacherbeit</option>
                      <option value="outside building">outside building</option>
                      <option value="Production">Production</option>
                      <option value="Production IR">Production IR</option>
                      <option value="Quality">Quality</option>
                      <option value="SI">SI</option>
                      <option value="TPT">TPT</option>
                      <option value="Video IR">Video IR</option>
                      <option value="VK">VK</option>
                      <option value="WC">WC</option>
                      <option value="ZB">ZB</option>
                      <option value="Zeta">Zeta</option>
                      <option value="ZSB">ZSB</option>
                    </>
                  ) : label === "Operation Mode" ? (
                    <>
                      <option value="">Select Operation mode</option>
                      <option value="Maintenance">Maintenance</option>
                      <option value="Malfunction">Malfunction</option>
                      <option value="Rosslyn">Normal mode</option>
                      <option value="Testing mode">Testing mode</option>
                      <option value="Normal operation">Normal operation</option>
                    </>
                  ) : label === "Hazard source" ? (
                    <>
                      <option>Select Hazard source</option>
                      <option value="Workplace / Work environment">
                        Workplace / Work environment
                      </option>
                      <option value="non adequate posture">
                        non adequate posture
                      </option>
                      <option value="parts with dangerous surface">
                        parts with dangerous surface
                      </option>
                      <option value="Tools / Utilities">
                        Tools / Utilities
                      </option>
                      <option value="Machine Tool">Machine Tool</option>
                      <option value="Slippy floor">Slippy floor</option>
                      <option value="Conveyor System / Robot">
                        Conveyor System / Robot
                      </option>
                      <option value="heavy object">heavy object</option>
                      <option value="non organized objects">
                        non organized objects
                      </option>
                      <option value="Power / Work Machine">
                        Power / Work Machine
                      </option>
                      <option value="Metal Working Machine">
                        Metal Working Machine
                      </option>
                      <option value="Packaging Machine">
                        Packaging Machine
                      </option>
                      <option value="Assembly table">Assembly table</option>
                      <option value="Processing Machines">
                        Processing Machines
                      </option>
                      <option value="even ground">even ground</option>
                      <option value="Work place / paths">
                        Work place / paths
                      </option>
                      <option value="road accident">road accident</option>
                      <option value="Work table">Work table</option>
                      <option value="Lifting Gear / conveyors">
                        Lifting Gear / conveyors
                      </option>
                      <option value="non adequate ladder">
                        non adequate ladder
                      </option>
                      <option value="non stable door">non stable door</option>
                      <option value="Power / Work machine">
                        Power / Work machine
                      </option>
                      <option value="Work place / stairs">
                        Work place / stairs
                      </option>
                      <option value="Industrial truck">Industrial truck</option>
                      <option value="Sharp tool">Sharp tool</option>
                      <option value="Wood and processing machines">
                        Wood and processing machines
                      </option>
                      <option value="Press machine">Press machine</option>
                      <option value="Plastics processing machines">
                        Plastics processing machines
                      </option>
                    </>
                  ) : label === "Machine Type" ? (
                    <>
                      <option>Select machine type</option>
                      <option value="Assembly line">Assembly line</option>
                      <option value="No involved Machine">
                        No involved Machine
                      </option>
                      <option value="cutting machine">cutting machine</option>
                      <option value="AGV SYSTEM">AGV SYSTEM</option>
                      <option value="tesa cutter">tesa cutter</option>
                      <option value="Ushall">Ushall</option>
                      <option value="Reach Truck">Reach Truck</option>
                      <option value="painting machine">painting machine</option>
                      <option value="Assembly table">Assembly table</option>
                      <option value="Machine CRMP CENTER 365">
                        Machine CRMP CENTER 365
                      </option>
                      <option value="Press MFU">Press MFU</option>
                      <option value="forklift">forklift</option>
                      <option value="SERTISSAGE (::)">SERTISSAGE (::)</option>
                      <option value="other">other</option>
                      <option value="Hanke">Hanke</option>
                      <option value="SCHÄFER">SCHÄFER</option>
                      <option value="Coupe tesa">Coupe tesa</option>
                      <option value="Hanke 971">Hanke 971</option>
                      <option value="plan de travail">plan de travail</option>
                      <option value="HANKE">HANKE</option>
                      <option value="chariot">chariot</option>
                      <option value="schaeffer">schaeffer</option>
                      <option value="trolley">trolley</option>
                      <option value="Komax alpha 355">Komax alpha 355</option>
                      <option value="schleuniger">schleuniger</option>
                      <option value="Staple gun">Staple gun</option>
                      <option value="Porte">Porte</option>
                      <option value="cutting mold">cutting mold</option>
                      <option value="Mold of foam injection">
                        Mold of foam injection
                      </option>
                      <option value="product transport trolley">
                        product transport trolley
                      </option>
                      <option value="Baubrettbeförderer">
                        Baubrettbeförderer
                      </option>
                      <option value="Baubrett">Baubrett</option>
                      <option value="workstation">workstation</option>
                      <option value="KOMAX">KOMAX</option>
                      <option value="couvercle de rProductionrk en bois">
                        couvercle de rProductionrk en bois
                      </option>
                      <option value="Foam injection robot">
                        Foam injection robot
                      </option>
                      <option value="Schleuniger">Schleuniger</option>
                      <option value="pistolet">pistolet</option>
                      <option value="slippery ground">slippery ground</option>
                      <option value="metal support">metal support</option>
                      <option value="marble">marble</option>
                      <option value="rearing trolley">rearing trolley</option>
                      <option value="cart">cart</option>
                      <option value="KSK Branch">KSK Branch</option>
                      <option value="Metal support of the water bottles">
                        Metal support of the water bottles
                      </option>
                      <option value="Palette">Palette</option>
                      <option value="Commission kit">Commission kit</option>
                      <option value="Gable">Gable</option>
                      <option value="Pistolet pneumatique">
                        Pistolet pneumatique
                      </option>
                      <option value="chariot elevateur">
                        chariot elevateur
                      </option>
                      <option value="Ameise">Ameise</option>
                      <option value="Ladders">Ladders</option>
                      <option value="Machine 061 schluniger">
                        Machine 061 schluniger
                      </option>
                      <option value="Schleuniger CrimpCenter 63">
                        Schleuniger CrimpCenter 63
                      </option>
                      <option value="Pistole">Pistole</option>
                      <option value="Trolley">Trolley</option>
                      <option value="Conveyor system">Conveyor system</option>
                      <option value="circular saw">circular saw</option>
                      <option value="Lagger">Lagger</option>
                      <option value="Transpalet">Transpalet</option>
                      <option value="Sewing">Sewing</option>
                      <option value="AP2_JLR">AP2_JLR</option>
                      <option value="Assembly">Assembly</option>
                      <option value="crimping machine">crimping machine</option>
                      <option value="Baubrett (Assembly line)">
                        Baubrett (Assembly line)
                      </option>
                      <option value="lifting machine">lifting machine</option>
                      <option value="Gamma">Gamma</option>
                      <option value="Electric screwdriver">
                        Electric screwdriver
                      </option>
                      <option value="stepladder">stepladder</option>
                      <option value="support metalique">
                        support metalique
                      </option>
                      <option value="plastic boxes">plastic boxes</option>
                      <option value="foaming ingection machine">
                        foaming ingection machine
                      </option>
                      <option value="Metal support">Metal support</option>
                      <option value="stepladder locking bracket">
                        stepladder locking bracket
                      </option>
                      <option value="shelfs">shelfs</option>
                      <option value="Electrical function test">
                        Electrical function test
                      </option>
                      <option value="Nutrunner">Nutrunner</option>
                      <option value="racks">racks</option>
                      <option value="support metallique">
                        support metallique
                      </option>
                      <option value="the Guillot support">
                        the Guillot support
                      </option>
                      <option value="the pince of boutel">
                        the pince of boutel
                      </option>
                      <option value="Tesa spender">Tesa spender</option>
                      <option value="Three-hole expansion pliers (HWZ)">
                        Three-hole expansion pliers (HWZ)
                      </option>
                      <option value="baubrett ">baubrett </option>
                      <option value="Schafer-736">Schafer-736</option>
                      <option value="Blade Holder">Blade Holder</option>
                      <option value="HANKE 11290/044">HANKE 11290/044</option>
                      <option value="Fire fighting door">
                        Fire fighting door
                      </option>
                      <option value="wooden pallet in the production process">
                        wooden pallet in the production process
                      </option>
                      <option value="KOMAX 253">KOMAX 253</option>
                      <option value="metal cart">metal cart</option>
                      <option value="IR table structure">
                        IR table structure
                      </option>
                      <option value="changing room floor">
                        changing room floor
                      </option>
                      <option value="plastic box">plastic box</option>
                      <option value="changing room">changing room</option>
                      <option value="foaming machine mold">
                        foaming machine mold
                      </option>
                      <option value="feeding workstation">
                        feeding workstation
                      </option>
                      <option value="Plastic box">Plastic box</option>
                      <option value="anti fatigue mat">anti fatigue mat</option>
                      <option value="cable connector">cable connector</option>
                    </>
                  ) : label === "Type of Injury" ? (
                    <>
                      <option value="">Select Type of Injury</option>
                      <option value="Abrasion">Abrasion </option>
                      <option value="Bruise">Bruise</option>
                      <option value="Burn/Scald">Burn/Scald</option>
                      <option value="Chemical burn">Chemical burn</option>
                      <option value="Cut">Cut</option>
                      <option value="Fracture">Fracture</option>
                      <option value="Laceration">Laceration</option>
                      <option value="Other">Other</option>
                      <option value="Sprain">Sprain</option>
                      <option value="Stab injury">Stab injury</option>
                      <option value="Stab wound">Stab wound</option>
                    </>
                  ) : label === "Injury Body Part" ? (
                    <>
                      <option value="">Select Injured Body Part</option>
                      <option value="Arm">Arm</option>
                      <option value="Body">Body</option>
                      <option value="Eye">Eye</option>
                      <option value="Hand">Hand</option>
                      <option value="Head">Head</option>
                      <option value="Internal organs">Internal organs</option>
                      <option value="Leg">Leg</option>
                      <option value="Other">Other</option>
                      <option value="Nose">Nose</option>
                    </>
                  ) : label === "Accident Type" ? (
                    <>
                      <option value="">Select Accident Type</option>
                      <option value="Non-work related accident">
                        Non-work related accident
                      </option>
                      <option value="Work related accident">
                        Work related accident
                      </option>
                    </>
                  ) : label === "Repeat incident" ? (
                    <>
                      <option value="">Select Repeated Accident</option>
                      <option value="Yes">Yes</option>
                      <option value="No">No</option>
                    </>
                  ) : label === "Maximum Possible Damage Occured" ? (
                    <>
                      <option value="">
                        Select Maximum Possible Damage/Harm Occurred
                      </option>
                      <option value="Yes">Yes</option>
                      <option value="No">No</option>
                    </>
                  ) : label === "Risk Factor" ? (
                    <>
                      <option value="">Select Risk factor</option>
                      <option value="1.1 - Workplace-related training">
                        1.1 - Workplace-related training
                      </option>
                      <option value="1.2 - Workplace-related operating instruction">
                        1.2 - Workplace-related operating instruction
                      </option>
                      <option value="1.3 - Coordination of work">
                        1.3 - Coordination of work
                      </option>
                      <option value="1.4 - Hazardous work">
                        1.4 - Hazardous work
                      </option>
                      <option value="1.5 - Use of personal protective equipment">
                        1.5 - Use of personal protective equipment
                      </option>
                      <option value="1.6 - First-Aid Systems">
                        1.6 - First-Aid Systems
                      </option>
                      <option value="1.7 - Alarm- and rescue measures">
                        1.7 - Alarm- and rescue measures
                      </option>
                      <option value="1.8 - Hygiene">1.8 - Hygiene</option>
                      <option value="1.9 - Organisation of Occupational safety and health">
                        1.9 - Organisation of Occupational safety and health
                      </option>
                      <option value="1.10 - General communication">
                        1.10 - General communication
                      </option>
                      <option value="1.11 - Mandatory testing of work equipment">
                        1.11 - Mandatory testing of work equipment
                      </option>
                      <option value="1.12 - Employment restrictions">
                        1.12 - Employment restrictions
                      </option>
                      <option value="2.1 - Working spaces">
                        2.1 - Working spaces
                      </option>
                      <option value="2.2 - Traffic routes">
                        2.2 - Traffic routes
                      </option>
                      <option value="2.3 - Falling on even ground, slipping, stumbling, twisting one’s ankle, missteps">
                        2.3 - Falling on even ground, slipping, stumbling,
                        twisting one’s ankle, missteps
                      </option>
                      <option value="2.4 - Falling from a height">
                        2.4 - Falling from a height
                      </option>
                      <option value="2.5 - Containers and confined spaces">
                        2.5 - Containers and confined spaces
                      </option>
                      <option value="2.6 - Working close to water">
                        2.6 - Working close to water
                      </option>
                      <option value="3.1 - Heavy physical work">
                        3.1 - Heavy physical work
                      </option>
                      <option value="3.2 - Physical work straining on one side">
                        3.2 - Physical work straining on one side
                      </option>
                      <option value="3.3 - Lighting">3.3 - Lighting</option>
                      <option value="3.4 - Climate">3.4 - Climate</option>
                      <option value="3.5 - Assimilation of information">
                        3.5 - Assimilation of information
                      </option>
                      <option value="3.6 - Age- and aging-based design">
                        3.6 - Age- and aging-based design
                      </option>
                      <option value="3.7 - Extent of perception">
                        3.7 - Extent of perception
                      </option>
                      <option value="3.8 - Impeded handling of work equipment">
                        3.8 - Impeded handling of work equipment
                      </option>
                      <option value="3.9 - Standing workplaces">
                        3.9 - Standing workplaces
                      </option>
                      <option value="3.10 - Workstations">
                        3.10 - Workstations
                      </option>
                      <option value="4.1 - Unprotected moving parts of machinery">
                        4.1 - Unprotected moving parts of machinery
                      </option>
                      <option value="4.2 - Parts with hazardous surfaces">
                        4.2 - Parts with hazardous surfaces
                      </option>
                      <option value="4.3 - Means of transport">
                        4.3 - Means of transport
                      </option>
                      <option value="4.4 - Parts moving uncontrolled">
                        4.4 - Parts moving uncontrolled
                      </option>
                      <option value="5.1 - Principles">5.1 - Principles</option>
                      <option value="5.2 - Hazardous body currents">
                        5.2 - Hazardous body currents
                      </option>
                      <option value="5.3 - Electric arcs">
                        5.3 - Electric arcs
                      </option>
                      <option value="6.1 - Harmful effects of substances (gases, vapours, aerosols, dusts, liquids, solids)">
                        6.1 - Harmful effects of substances (gases, vapours,
                        aerosols, dusts, liquids, solids)
                      </option>
                      <option value="6.2 - Skin exposure">
                        6.2 - Skin exposure
                      </option>
                      <option value="6.3 - Miscellaneous impacts (odours/oxygen deficiency)">
                        6.3 - Miscellaneous impacts (odours/oxygen deficiency)
                      </option>
                      <option value="7.1 - Fire hazards related to solids, fluids and gases">
                        7.1 - Fire hazards related to solids, fluids and gases
                      </option>
                      <option value="7.2 - Hazard due to explosive atmosphere">
                        7.2 - Hazard due to explosive atmosphere
                      </option>
                      <option value="7.3 - Thermal explosions (runaway reactions)">
                        7.3 - Thermal explosions (runaway reactions)
                      </option>
                      <option value="7.4 - Physical explosions (e.g. due to hot metal melts and water)">
                        7.4 - Physical explosions (e.g. due to hot metal melts
                        and water)
                      </option>
                      <option value="7.5 - Explosive substances (explosives)">
                        7.5 - Explosive substances (explosives)
                      </option>
                      <option value="7.6 - Miscellaneous explosive material (i.e. peroxides)">
                        7.6 - Miscellaneous explosive material (i.e. peroxides)
                      </option>
                      <option value="8.1 - Targeted activities">
                        8.1 - Targeted activities
                      </option>
                      <option value="8.2 - Non-targeted activities">
                        8.2 - Non-targeted activities
                      </option>
                      <option value="9.1 - Noise">9.1 - Noise</option>
                      <option value="9.2 - Ultrasound">9.2 - Ultrasound</option>
                      <option value="9.3 - Whole-body vibration">
                        9.3 - Whole-body vibration
                      </option>
                      <option value="9.4 - Hand-transmitted vibrations">
                        9.4 - Hand-transmitted vibrations
                      </option>
                      <option value="9.5 - Non-ionising radiation">
                        9.5 - Non-ionising radiation
                      </option>
                      <option value="9.6 - Ionising radiation">
                        9.6 - Ionising radiation
                      </option>
                      <option value="9.7 - Electromagnetic hazards">
                        9.7 - Electromagnetic hazards
                      </option>
                      <option value="9.8 - Contacts with hot or cold media">
                        9.8 - Contacts with hot or cold media
                      </option>
                      <option value="9.9 - Electrostatic hazards">
                        9.9 - Electrostatic hazards
                      </option>
                      <option value="9.10 - Overpressure">
                        9.10 - Overpressure
                      </option>
                      <option value="10.1 - Mental overload or underload">
                        10.1 - Mental overload or underload
                      </option>
                      <option value="10.2 - Scope of action, responsibility">
                        10.2 - Scope of action, responsibility
                      </option>
                      <option value="10.3 - Social conditions">
                        10.3 - Social conditions
                      </option>
                      <option value="10.4 - Regulation of working hours">
                        10.4 - Regulation of working hours
                      </option>
                      <option value="10.5 - Misuse of alcohol- or drugs">
                        10.5 - Misuse of alcohol- or drugs
                      </option>
                      <option value="11.1 - Field work">
                        11.1 - Field work
                      </option>
                      <option value="11.2 - Humans">11.2 - Humans</option>
                      <option value="11.3 - Animals">11.3 - Animals</option>
                      <option value="11.4 - Plants">11.4 - Plants</option>
                      <option value="11.5 - Environmental impacts">
                        11.5 - Environmental impacts
                      </option>
                    </>
                  ) : label === "Date" ? (
                    <input type="date" className="date-input" />
                  ) : label === "Lost Days" ? (
                    <input type="number" className="number-input" />
                  ) : label === "Recruitment date" ? (
                    <input type="date" className="date-input" />
                  ) : label === "SMS" ? (
                    <>
                      <option value="">Select SMS</option>
                      <option value="Yes">Yes</option>
                      <option value="No">No</option>
                    </>
                  ) : label === "Accident shift" ? (
                    <>
                      <option value="">Select Accident shift</option>
                      <option value="Shift1">Shift1</option>
                      <option value="Shift2">Shift2</option>
                      <option value="Shift3">Shift3</option>
                    </>
                  ) : (
                    <option value="" disabled>{`Select ${label}`}</option>
                  )}
                </select>
              </div>
            ))}
          </div>
          <button className="sendAlert-btn">Send alert</button>
        </div>
      </div>
    </div>
  );
}
